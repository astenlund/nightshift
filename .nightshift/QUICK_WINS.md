...
### Let one cumulative assessment cover code and docs tasks together

Observed in run `6e70931a-3760-46e2-b628-fdaa4a29f0e3` on 2026-09-25 (plugin 3.2.7, Claude Code controller), which had a code task followed by a dependent docs task. After the docs task changed files the code task's clean assessment had covered, `complete` refused with `stale-review`, and a fresh, complete, strong and broad cumulative `code` assessment dispatched on the docs task (`b151044b`) did not satisfy the code task's gate. The identical assessment had to be dispatched again on the code task. The mirror case followed after a later repair: the docs task's assessment went stale, and the code task's fresh cumulative assessment did not count for it, so one run needed two identical cumulative assessments per invalidation. This is the current design rather than a defect: `dispatch` in `internal/runtime/cli.js` carries other completed tasks' ids and commitments only into a code task's assessment and only for code tasks, `reviewGateFailure` in `internal/runtime/lifecycle.js` admits another task's review only when the task, the owner and the review kind are all `code` and the review lists the task, admitting only a task's own reviews otherwise, and `internal/runtime/REFERENCE.md` describes cumulative coverage for code tasks only. A docs-task dispatch therefore carries neither the code task's id nor its commitments, so dropping the owner-kind check alone would not have helped. The gate fails closed, so the cost is an extra full review round, not incorrect completion. The user chose to track it at triage.

Let one cumulative `code`-kind assessment carry and satisfy the commitments of the code and docs tasks it covers, whichever of the two owns it, changing the dispatch filter and the gate's admission filter together so a covering review always lists each covered task and includes its commitments and required dimensions. Spec tasks stay excluded on both sides: the gate keeps only the latest admitted candidate, so a covering `code` review admitted for a spec task would displace its own spec assessment and fail the spec dimensions. No existing test covers that displacement: the tests in `tests/runtime-regressions.test.js` asserting that a later cumulative code review cannot replace an earlier governing-spec assessment use only code tasks with a pre-reviewed spec, and no test imports a covering `code` review for a spec task, so the spec-exclusion test must import a later covering `code` review that lists a completed spec task and assert that the spec task's gate still holds. Whether lore tasks, which also take `code`-kind reviews, join the covered set is unsettled. The covering review replaces only the duplicate assessment: each covered task still needs its own fresh checks and resolved findings, and the covering review must postdate its latest requirements revision and repairs, so a check whose paths the other task changed is still re-run on its own task. Add runtime tests for both directions, the spec exclusion and the per-task check freshness case, and update `REFERENCE.md` to match. [Closing tracking review](features/closing-tracking-review.md) also changes which task reviews the completion gate accepts, so whichever lands second reconciles with the other. Runtime changes ship with a version increase. Tracking does not authorize implementation.
