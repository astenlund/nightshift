...
The local 3.1.0 cache-replacement candidate has observations for all twenty acceptance items on both supported hosts. Final cumulative assessment of the complete delivery and this updated report remains blocked by automatic export approval review. The bug stays active until that assurance and final closure are complete; publication is not authorized. This report accompanies [the governing design](../specs/retained-plugin-releases.md); durable run `2a89bde5-9c0c-4ba5-b67f-dadc545bfcc4` owns current workflow state.
...
The final cumulative Astra dispatch was rejected before launch twice. The second attempt supplied the earlier explicit approval for full cumulative source, global/project instructions, selected native evidence and required follow-ups, plus the later isolated-context and model-fallback approvals. Automatic approval review still required a fresh trusted user authorization for this exact payload and destination. No reviewer ran and no additional review tokens were charged. The prepared assessment excludes credentials and unrelated host configuration, and uses the configured Codex/OpenAI Astra destination. This approval-system rejection is a remaining capability gate, not a failed native acceptance case or a completed assurance pass.
