...
Implementation authorized on 2026-09-07; candidate work is in progress. This feature consolidates the agreed [vision](../../VISION.md), [workflow](../../WORKFLOW.md), and [migration assessment](../../V3-MIGRATION.md). It defines the first v3 release. Required acceptance evidence is still incomplete, and no v3 capability is claimed as shipped.
...
The approved budget for the initial v3 MVP test run is 2,000,000 tokens total across both hosts and every participating test agent, including compaction testing. The user subsequently raised the total campaign allowance in stages to 4,000,000, 8,000,000, 16,000,000, 32,000,000, 64,000,000, 128,000,000 and 256,000,000 tokens, retaining all earlier usage and uncertainty reservations. This is a run-specific cap, not a permanent product default. For the 256M continuation, favor Claude/Fable for execution and independent reviews, conserving Codex for the remaining host-specific check. This allocation applies to this acceptance run. Start with a small probe on each host, run comprehensive acceptance once the candidate is coherent, then rerun affected cases for failures or relevant changes. Another comprehensive run needs a concrete reason and must fit the remaining budget. Editorial documentation changes alone do not trigger expensive repetition; changes to active skill instructions or prompts are behavior changes and require appropriate revalidation. Budget exhaustion leaves failed or unverified requirements incomplete; increasing the cap requires the user's direction.
...
The [candidate acceptance report](../reports/v3-acceptance-128m-20260909.md) continues the earlier checkpoints with independently verified Codex retrospective approval/application, attended stop/resumption and ordinary review-loop behavior. The material-intake case reaches final approval but remains unfinished at the 128M budget guard; remaining Claude cases also require renewed authentication. Historical failed outcomes remain intact beside independently verified adjudications. No incomplete requirement is treated as passed.
