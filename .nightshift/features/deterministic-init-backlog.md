...
- Four active indexes: `.nightshift/QUICK_WINS.md`, `.nightshift/FEATURES.md`, `.nightshift/BUGS.md`, and `.nightshift/PATTERNS.md`.
- Three history archives: `.nightshift/QUICK_WINS_HISTORY.md`, `.nightshift/FEATURES_HISTORY.md`, and `.nightshift/BUGS_HISTORY.md`.
- The structural `.claude/` parent and `.nightshift/features/`, `.nightshift/bugs/`, `.nightshift/patterns/`, and `.nightshift/plans/`.
...
- The unconditional repository-local `.nightshift/plans/` ignore policy in Git repositories.
- The fresh-scaffold track, ignore, or defer election for the remaining backlog surface.
- Hard-wrap inspection and repair for the seven top-level backlog files and Markdown files under `.nightshift/features/`, `.nightshift/bugs/`, and `.nightshift/patterns/`.
- Mixed-line-ending inspection and normalization for that same backlog-file surface.

Other top-level Markdown files and `.nightshift/plans/` contents remain outside mechanical repair scope. `.claude/quick_wins/` and Quick Win breakout prose remain outside this feature until Pick-time breakouts extends the controller.

The controller does not migrate populated implemented or fixed sections into history archives, change an existing project from tracked to ignored, run mutating Git commands, add or remove index entries, autonomously author or alter user-controlled semantic prose, decide whether customized prose satisfies a concept, or generalize host-neutral routing beyond init-backlog. It may publish an exact model-authored semantic repair or a mechanical backlog-file repair only when that effect is included in the complete manifest shown to and approved by the user.

## Ownership and architecture

`skills/init-backlog/init-backlog.js` is the private deterministic controller. It owns target discovery, root-guidance resolution, template loading, inspection, deterministic proposals, snapshot binding, effect validation, filesystem effects, Git-aware policy inspection, backlog-file repair execution, recovery, and final ready-parser validation.
...
A present file is exact-template only when its complete logical content equals the composed template. Customized surrounding prose is not silently treated as template-owned. Existing target bytes, BOM state, newline convention, mode where meaningful, and controlled semantic regions are inspection facts and are preserved or changed only by the exact proposal shown for approval. Mixed LF and CRLF endings are a repairable inspection state only on the controlled backlog-file surface; they are not an acceptable steady state.
...
3. Present every proposed effect, unresolved condition, safety disclosure, and election choice. Semantic exact edits and whole-file mechanical proposals show exact decoded before and after content. Only a mechanical repair of breakout prose may substitute a deterministic digest carrier because breakout content has no semantic-decision role in this feature, while top-level backlog files and root guidance can require semantic classification whose exact content must remain reviewable. The carrier identifies the target, destination newline style, whether hard-wrap repair is composed into the same output, and exact before and after identities, and discloses that the prose images are withheld. No other effect may replace exact content with digest-only disclosure.
...
The controller predicts exact physical output from the inspected target bytes, the template's logical content, Git attributes and configuration where applicable, and the approved action. It never round-trips a Windows checkout through newline-translating text mode. A supported-size controlled backlog file with mixed LF and CRLF endings produces a mechanical repair proposal instead of aborting inspection. The proposal selects one uniform destination style through the existing per-target newline policy, preserves the BOM and terminal-newline state, changes no non-terminator byte unless a hard-wrap repair is composed into the same predicted output, and participates in the normal approval, snapshot, recovery, and verification lifecycle. When the policy reports `choice-required`, the skill presents LF and CRLF alternatives and obtains the user's preference; denial or an unavailable user leaves the file unchanged and the run incomplete. A deferred semantic decision or pre-existing structural defect does not suppress the mechanical repair; the target remains incomplete for that separate reason, exposes no semantic edit region while structurally invalid, and receives fresh region offsets after a structurally valid repair. Invalid UTF-8, bare carriage returns, unsafe BOM placement, mixed endings outside the controlled backlog-file surface, or an indeterminate Git conversion still fails before writing that target. The 262,144-byte mechanical input and predicted-output limit remains an independent resource boundary: an oversized bug or pattern breakout fails inspection, an oversized feature breakout remains opaque and produces a blocking runtime-state problem, and newline expansion beyond the output bound fails before approval, so no unknown or unpublishable line-ending state is accepted as complete.
...
The implementation adds fixture-driven controller tests and exactly one new project suite while preserving the ready and unwrap suites and their public CLIs. Verification covers the complete supported-size controlled surface, missing and customized targets, semantic regions, strict content validation, path confinement, aliases and links, Git and non-Git classification, track, ignore, defer, denial, unavailable-user, and conflict branches, LF and CRLF materialization, mixed-line-ending normalization to each style, the genuinely ambiguous preference branch, composition with hard-wrap repair, BOM and terminal-newline preservation, semantic-region rescanning, structurally invalid and semantically deferred targets, exact mechanical size boundaries, oversized blocking behavior, snapshot drift, response-loss resume, partial multi-write states, restoration, stale ownership, marker recovery, abandoned backups, and cleanup failure on Windows. Injected-platform cases retain fail-closed coverage for inert POSIX branches; the POSIX-gated permission test and real-filesystem POSIX mode assertions are outside the Windows-only live verification matrix. Verification separately proves that structural directories, plans content, or unrelated host configuration without controlled non-plan backlog evidence still opens the fresh election, while any controlled non-plan backlog file without a valid marker retains historical policy. It also proves that an independently authorized action can land and return incomplete beside an unrelated deferred decision, while an action connected to that unresolved decision remains blocked.
...
- revise-spec refreshed 2026-09-03 15:50 at 4bb64ae, scope: whole file, content: aeaae01d (live-claim marker migration)
