...
Canonical locations are `.claude/quick_wins/<slug>.md`, `.nightshift/features/<slug>.md`, and `.nightshift/bugs/<slug>.md`. A deterministic slug is derived from the existing title. If the canonical path already belongs to a different source identity, materialization fails closed and asks the user for a different slug rather than overwriting or silently suffixing the path.
