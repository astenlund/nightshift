...
- The spec authoring shape this feature extends: the `Feature:` first line and `## What it does` convention in `.nightshift/features/` house shape (existing).
...
- The orchestration transition module and fixtures (`internal/revise/orchestration.js`, `internal/revise/orchestration.test.js`; shipped, landed before this feature): `resolveBoundary` encodes a closed two-trigger clearing set (staleness by fingerprint inequality, selective per-cell scope remap via the `remap` input), and anchor revision is a third global trigger that is neither. Two candidate shapes, decided in this feature's spec run: (a) extend the module's clearing enumeration, the affected result carriers, and the fixtures atomically; or (b) fold the anchor into the fingerprint instead: for plan and code runs, widen the controller's fingerprint input to hash the governing anchor bytes alongside the reviewed content, so anchor revision rides the existing staleness trigger and stamp conjunction with no module change (for spec runs the anchor is part of the artifact and already moves the fingerprint). The remap trigger stays a separate input either way: it is deliberately per-cell selective, and folding the scope map into a global fingerprint would discard unaffected certifications. Option (b) touches the fingerprint recipe, so it coordinates with content-fingerprint-helper's selector taxonomy if both are in flight.
