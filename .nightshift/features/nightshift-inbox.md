...
Settled by user ruling on 2026-09-03: the inbox folder is git-ignored, the same treatment `.nightshift/plans/` and `.tmp/` get, so drops never enter history and never touch the tracked backlog. Only promotion at triage writes tracked files.
