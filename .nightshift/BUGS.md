...
Local candidate progress: the [approved executive scope](specs/run-adoption-and-continuation-executive.md) governs an implementation described in the [candidate verification report](reports/run-adoption-and-continuation-20260921.md). Installed takeover, continuation and most refusal cases are observed. The Claude unavailable-claim and Codex unknown-controller refusal pair remains required and awaits a live-budget decision, so this entry is not yet resolved.
