...
The confirmed defect is terminating an unfinished reviewer on insufficient progress evidence. The zero-length readings are unexplained; delayed persistence, filesystem metadata observation and tool-output behavior are hypotheses only. Investigate which runtime liveness and progress signals a controller should rely on before declaring a stall, and establish the cause of the discrepant observations. Preserve genuine termination of stalled workers and the fallback path. Tracking does not authorize implementation.
