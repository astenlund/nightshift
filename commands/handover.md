...
Build a single flat TaskCreate queue from the detected entry step onward before starting work; mark each step completed as its sub-skill graduates and pull through to completion. Accumulate user-decision-required items (`/nightshift:revise-spec` and `/nightshift:revise-plan` findings deferred for later, implementation-phase questions and out-of-scope findings, `/nightshift:revise-code` tech-debt findings) into a single "follow-up items" list maintained in conversation memory; the morning report (step 12) presents this list. Sub-skill review checkpoints (spec or plan review after a revise loop, follow-up triage, end-of-loop asks) are deferred to that same morning report rather than pausing the run mid-pipeline: handover exists so the user can walk away, so each such checkpoint is recorded with what would have been asked and answered there. The run still stops for the shift-start confirm line, for destructive or irreversible actions, and when work is genuinely blocked on missing input. Brainstorming sign-off keeps its normal interactive flow, because producing an approved design is the human's work and handover never starts below it.
...
**Validate before proceeding.** Between resolving the artifacts and the stated conclusion, dispatch one fresh agent to validate the governing spec (or backlog entries) and plan against the current repo: does the described problem, design, and every file reference still hold? Stamps prove a hardening loop ran when the stamp was written; this validates content against today's tree, and for backlog-entry specs, which carry no stamps, it is the only currency check available. Fold the findings into the stated conclusion; validation findings are one of the signals that downgrade a clean detection into a confirm-first one.

Walk the ladder top-down and enter the procedure at the first rung whose condition holds. State the conclusion in one line, naming the chosen spec/plan paths, the target scope, and any stamp or staleness details, for example: "Spec .claude/specs/foo.md hardened for sections 3-5 (stamped 2026-07-04 09:41 at a1b2c3d), matching the target scope; no plan found; taking over from planning." When the detection is clean — the rung decided on solid evidence, artifact selection unambiguous, validation found no issues, and no staleness, drift, or inconclusive-evidence flags are raised — proceed directly to building the task queue: the stated conclusion is the user's interrupt point, not a question. Any flag, ambiguity, or validation finding instead asks the user to confirm before work starts ("... Correct?"), and the user's answer always overrides inference. References to "the confirm line" throughout this command mean this stated conclusion; where a flag applies, it is posed as the question just described.
...
1. **Spec gate.** `/nightshift:revise-spec` scoped to the target scope, run to graduation per the skill's own criteria. The skill's user review of the hardened spec is auto-passed per the unattended rule above; surface the noteworthy hardening deltas in the morning report instead. Keeping this step ahead of planning puts the natural context-compaction boundary here: run it at the brainstorm tail while the design context is warm, then compact, and a fresh handover session starts directly at planning with the spec already hardened. Findings flow into the follow-up items list.
