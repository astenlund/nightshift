...
**Status:** Nightshift 3.2.1 is in development, clarifying that an unavailable strong reviewer pauses further repairs unless a permitted strong substitute is secured, and that advisory assessment does not supply required review coverage. Its published baseline is 3.2.0, which added an in-place handover of a run already underway, a clear handover acknowledgment and a recorded morning report whose delivery is runtime state. The [handover acceptance report](.nightshift/reports/handover-transition-and-morning-report-20260919.md) records its independent assessments, the installed-host observations on both supported hosts and the branches that remain qualified. Version 3.1.1 added automatic first-use preparation and Ready/Exploring without continuation activation, and 3.1.0 introduced retained resources that survive plugin-cache replacement. The [retained-release acceptance report](.nightshift/reports/retained-plugin-releases-20260914.md) records completed implementation assessment, native observations on both supported hosts and their qualifications. The earlier [self-hosting report](.nightshift/reports/self-hosting-lifecycle-activation-20260913.md), [Ready presentation probes](.nightshift/reports/ready-exploring-presentation-20260912.md) and [v3 MVP acceptance report](.nightshift/reports/v3-acceptance-272m-20260910.md) preserve preceding evidence. See [VISION.md](VISION.md), [WORKFLOW.md](WORKFLOW.md) and the [v3 feature](.nightshift/features/nightshift-v3.md) for the supported scope.
...
Native persistent goals are the preferred continuation mechanism. Before unattended work, establish that the actual host's continuation and trusted hooks work. Handing over a run already underway records the handover and switches it to unattended mode in place; the controller then says plainly that the handover is accepted. A handed-over run owes a morning report that stands on its own: the runtime requires it before completion, records its delivery from your actual reply, and reminds the owning session at its next start while the report is undelivered or a follow-up decision is pending. Stop reminders resist premature yields in unattended mode and bound repeated reminders without recorded progress; exhaustion is an incomplete recovery result, never successful completion. A run whose only remaining blockers are user decisions may pause to ask, and a running dispatch is awaited inside the turn with the runtime wait operation. Attended runs may yield for conversation.
