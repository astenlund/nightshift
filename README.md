...
3. **Pick work** with `/nightshift:ready`, which resolves the declared dependency graph (via a deterministic, fixture-tested parser) and reports what's unblocked, what's blocked and on what, and any structural errors in the backlog, closing with a few argued candidates rather than an undifferentiated list. The report nominates work; it never authorizes it. Picking an entry, from the report or straight from an index, is an implementation entry that runs `/nightshift:spec-agreement` over that entry first, since a backlog entry counts as a spec and readiness is not agreement. `/nightshift:exploring` renders the `## Exploring` drafts in full; a draft routes to brainstorming to graduate before it can be worked at all.
