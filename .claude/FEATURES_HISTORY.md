...
- [Surface exploring entries in ready](features/ready-exploring-visibility.md): the ready parser now collects `## Exploring` drafts into an always-present `exploring` output array (`{index, title, link, excerpt}`, link verbatim and `null` when the heading is unlinked or its target empty), and their breakout links join the broken-link notice check with a draft-specific tail replacing the false "Requires line still resolves normally" clause. Two views project that one array: `/nightshift:ready` lists drafts titles-only in a not-ready section and now also surfaces `indexes.missing`, while the new `skills/exploring/SKILL.md` renders the full draft list and every parser problem channel (first landed as a command, moved to a skill at morning-report triage so both views of one array are the same host-portable artifact kind). Drafts stay out of the entry registry, so a `Requires:` reference pointing at one remains a structural error. Shipped 2026-08-15 in the 2.4.0 batch (commits 82a2908..635353f).
