...
- Staleness sweep: when every applicable cell is inactive, compare each cell's certified fingerprint with the current fingerprint. Reactivate exactly the cells whose certification is older, clearing each reactivated cell's certification, and start the next round. Staleness is evaluated only at this all-inactive boundary, never mid-round: accumulated changes batch into one wave instead of re-running settled cells per small delta. No cell is ever reactivated directly by another cell's edit or disposition; only the sweep reactivates.
...
- Verifier round: a round whose single cell is `verifier/whole-artifact`, reviewing the entire artifact at the wave-converged fingerprint under the holistic gate rules below. A clean verifier conclusion, or an all-refuted verifier round, stamps that exact fingerprint. Verifier findings follow the normal skeptic and adjudication pipeline; a verifier boundary that applies at least one fix increments the fingerprint once like any round boundary, leaves the stamp unset, and returns the run to the staleness sweep.
- A newly contradicted N/A justification becomes applicable immediately, active with no certification, so the run cannot complete without reviewing it. Re-evaluate every N/A declaration at each staleness sweep.

The run enters `post-review` only on the conjunction of wave convergence and a verifier stamp equal to the current fingerprint. The stamp is a conjunction, not an authority: the verifier never launches before wave convergence, so no single agent and no cap path can complete the run alone. Any reviewable-content edit after a stamp moves the fingerprint, which simultaneously invalidates the stamp and stales certifications; the staleness sweep is the single re-entry path.
```
...
- `Reactivate stale cells`: applies only when every applicable cell is inactive and at least one certification is older than the current fingerprint. In one atomic state rewrite, set every stale cell `Status: active` with `Certified fingerprint: none`, and re-evaluate every N/A declaration, changing a contradicted N/A to active with no certification and preserving an inapplicable cell as N/A only after re-evaluating and encoding its nonblank reason. Then `Start round`.
...
Adjudicate verifier findings with the standard disposition rules. At the verifier boundary: if at least one fix was applied, the fingerprint increments once, spawned consequences and applied changes are recorded exactly as at a reviewer round boundary, `Verifier stamp` stays `none`, and the staleness sweep resumes the run. If the verifier is clean, or every skeptic-verified verifier finding was refuted or accepted without an actionable follow-up, one atomic rewrite records `Verifier stamp` as the current fingerprint and enters `post-review`, with the note or skeptic evidence as the stamp rationale. Valid-but-deferred verifier findings create authoritative open Follow-ups rows like any other; they do not block the stamp.
...
| `Re-evaluate applicability immediately: a contradicted N/A becomes active with zero attempts, the finding's active cell remains active, and inactive cells remain inactive until the next mandatory phase. Refresh the fingerprint, map, payloads, union proof, and cumulative patch before the next round. The next phase starts every applicable cell active against the expanded final scope.` | `Re-evaluate applicability immediately: a contradicted N/A becomes active with no certification, the finding's active cell remains active, and inactive cells keep their certifications. Refresh the fingerprint, map, payloads, union proof, and cumulative patch before the next round; the moved fingerprint stales prior certifications, so the staleness sweep re-reviews every settled cell against the expanded scope.` |
| `An unchanged logical `(dimension, cluster)` cell keeps its ID, status, N/A reason, and phase attempt count when its label or membership changes.` | `An unchanged logical `(dimension, cluster)` cell keeps its ID, status, N/A reason, and certification when its label or membership changes.` |
...
| `An applicable cell becomes active if any applicable predecessor or same-dimension sibling is active, otherwise inactive for the rest of the phase if any is inactive, otherwise active.` | `An applicable cell becomes active if any applicable predecessor or same-dimension sibling is active; otherwise, if every applicable predecessor is inactive with a certification, it becomes inactive carrying the ordinal-minimum predecessor certification; otherwise it becomes active with no certification.` |
...
| the whole `## Why the completion guarantee is unaffected` section body | Rewrite as: `Post-review entry requires wave convergence plus the verifier stamp on the same fingerprint. Because a narrowed payload is delivered only after own-dimension fixes were applied, and applied fixes move the fingerprint, a cell that received a narrowed payload cannot hold a current certification without a later full-payload review; the wave model therefore forces full-coverage certification of the final fingerprint regardless of narrowing, and the verifier gate adds its independent holistic pass on top.` |
| `deferred coverage is picked up by the next phase's whole-scope reset` (and any remaining `next phase` phrasing found by grep) | `deferred coverage is picked up by the reactivation wave` |
