...
The schema would pin the report shape the controller consumes (findings list, per-finding fields, verdicts), let the reviewer validate its own output before finalizing, and give the controller a deterministic parse instead of a salvage attempt. Open questions for later design: whether the schema lives in the internal revise engine's bundled files, how validation runs on the agent side, and what the contract is when a reviewer cannot validate its own output (retry, re-dispatch, or degraded read).
