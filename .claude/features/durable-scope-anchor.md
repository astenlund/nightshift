...
- **Calibrate first-draft rigor to deployment context** (a not-yet-migrated proposal): the natural home for operating-context profiling, including audience ("who is this for"). Per the 2026-08-11 migration decision, this feature deliberately leaves operating context to that proposal and confines itself to outcome plus exclusions; the two anchor areas combine when that feature lands. A candidate audience category set (single-dev/personal use, trusted circle, paying customers, organization, public) is proposed there, and the deferral is noted at the top of that proposal's migration text with a pointer back to this file.
