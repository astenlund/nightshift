...
- **Initial-spec gate.** Right after the controller writes the spec and the user approves its decision-complete digest in the current session, before hardening. Catches a bad requirements read or structural mis-sketch before the dimension swarm iterates on it.
- **Hardened-spec gate.** After the review stage converges and the user approves the current post-review digest. This is the strong-model read on the exact artifact that will be handed off (the final invariant), and it subsumes the holistic third-phase reviewer role.

A finding-driven design edit invalidates current approval. Before another second-opinion read, revise loop, planning step, or implementation transition, control returns to the agreement gate and waits for approval of the refreshed digest. Review certification and user authorization remain separate: neither substitutes for the other.
