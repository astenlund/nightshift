...
a session-side check compares it against the latest version from the
...
in the template itself); which check surfaces the notification (the
SessionStart hook, `/nightshift:ready`, a dedicated `init-backlog
--check`, or a drift report); how the version interacts with the existing
