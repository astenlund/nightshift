...
- **Move generic release and conformance assertions out of the agreement controller suite.** The agreement controller test suite also covers generic repository-release state and command/CLI conformance, so version bumps and doc changes unnecessarily edit feature-specific tests. Preferred shape: relocate generic assertions to a generic suite that exercises release and command surface, so browsing the feature tests stays feature-scoped. The review-orchestration-tests ship (2.5.4 batch) already exercised this smell: its change had to update the suite-count pin (three sites) and version literals (two sites) inside `spec-agreement.test.js`; the relocation moves those now-seven-suite pins and release assertions to the generic suite, and any feature landing before it re-derives the count at its then-current home.
