...
- **Condition the missing-breakout notice tail on the entry's own classification.**
  ...
  own classification ended in a structural error. Every structural path co-occurs with
  a missing breakout file, since `addBreakoutTarget` runs regardless of the outcome: a
  non-sliced entry with no Requires line is the starkest case, and a broken link, bare
  text, a stale struck-slice reference, an External grammar error, and the two sliced
  pre-checks all reach the same tail. Preferred shape: pass the entry's classification
  outcome into the notice builder and word the tail from it. The outcome is derived
  from the entry's own `classifyTrackedEntry` call, the one that immediately precedes
  `addBreakoutTarget` in `classifyTrackedEntries`, never by scanning the finished
  `structuralErrors` array by title (`titleDupes` exists because two indexes can share
  a title). It has three values, checked in this order: `structural` when that call
  filed a structural error under the entry's title or one of its bracketed slice titles
  (a sliced entry classifies once per unshipped slice, so a ready slice beside a
  structural slice counts as `structural`); else `cycle` when the entry is a
  dependency-cycle member (today it classifies to nothing under its own title because
  `classifyUnit` returns on `unit.excluded` and the cycle error is filed under
  `[cycle]`; once [Validate the External line of a dependency-cycle
  member](#ready-parser-precision) lands, an excluded member with a malformed External
  line files its own structural error and takes `structural` by this precedence); else
  `resolved`. Only `resolved` keeps the "still resolves normally" tail, `structural`
  says the entry's own classification already reports a structural error, and `cycle`
  points at the cycle error. The outcome rides on the `breakoutTargets` record beside
  the existing `draft` flag and is omitted when `resolved`, mirroring how `draft` is
  carried by presence, so a hand-built rec without it (`scanBreakoutTargets` is called
  directly with such recs in `ready.test.js`) takes the `resolved` tail; the
  byte-for-byte JSON-stability assertion in `ready.test.js` is unaffected for a
  resolved entry and gains no field. Fixtures: one non-sliced structural-error entry,
  one cycle member, and one sliced entry mixing a ready slice with a structural slice,
  each with a missing breakout file. Surfaced by the breakout-dependency-drift spec
  review on 2026-08-22; pre-existing imprecision outside that fix's scope. Touches
  `scanBreakoutTargets`, which [Break the agreement controller and its ready dependency
  cycle into narrower scanner and controller modules](#agreement-gate-follow-ups-deferred-during-present-spec-for-agreement-revise-code)
  relocates; land this before that split or rebase onto it.
- **Validate the External line of a dependency-cycle member.** `classifyUnit` in
  ...
  fixed (one extra round trip, self-healing). The same return also hides a malformed
  inline `**Requires:**` on a continuation slice of a sliced cycle member, a line the
  cycle error never owns (`collectEntryEdges` reads only the top-level line). Preferred
  shape: whole-entry exclusion suppresses classification, not grammar validation. Order
  `classifyUnit` as missing-Requires return, then the Requires loop, then the External
  loop, then the existing single joined structural push (Requires problems first, as
  today, so a non-excluded unit's report is byte-identical), then the exclusion return,
  then the blocked/external/ready classification. An excluded unit therefore files its
  grammar problems under its own title and nothing else: no Blocked entry (its links
  resolve to labels that are discarded) and no External classification for a
  well-formed line (pinned today by `ready.test.js`'s "a cycle member's external
  blocker is subsumed by whole-entry exclusion"). The missing-Requires return stays
  first so a non-excluded entry with no Requires line still reports only the
  missing-line problem (a null Requires line sources no edges, so a cycle member never
  takes that return). Fixtures: a two-entry cycle with a link in External on one
  member, the same cycle with a malformed inline External and a bare-text inline
  Requires on a continuation slice of a sliced member (both problems reported in one
  error under the bracketed slice title, no Blocked entry), and an entry with no
  Requires line plus `**External:** none.` pinning the single missing-line error.
  Surfaced by the breakout-dependency-drift final review on 2026-08-22. Touches
  `classifyUnit`, which [Break the agreement controller and its ready dependency cycle into narrower scanner
  and controller modules](#agreement-gate-follow-ups-deferred-during-present-spec-for-agreement-revise-code)
  relocates; land this before that split or rebase onto it. It also lets a cycle
  member file a structural error under its own title, which [Condition the
  missing-breakout notice tail on the entry's own
  classification](#ready-parser-precision) resolves through its
  `structural`-before-`cycle` precedence; the cycle-member notice fixture there must
  hold in either landing order.
