...
While the marker is `fit-check` or `agreement`, prohibit every reviewer, skeptic, verifier, post-review, planning, implementation, or downstream dispatch. Also block those dispatches while a controller mutation remains pending. A pending user request blocks round and verifier launches only: a repair launch and the round-internal skeptic fan-out proceed under a pending user request, because the drain is gated on full round evaluation, which can require exactly that work (`preflightLaunch` in `orchestration.js` encodes the repair exemption; the skeptic fan-out is round-internal dispatch outside its launch-kind enumeration). All reconstruction, classification, and volatile-state storage failures retain the last non-`none` marker and dispatch nothing. If the first `fit-check` rewrite itself fails while `none` remains on disk, the durable applied-change ledger or pending request still makes the batch unresolved; retry `fit-check` before any dispatch. Any later failed boundary rewrite leaves the prior successfully written non-`none` marker authoritative.
