...
2. **Derive.** When absent or skeletal, derive the section from durable project knowledge first (repository guidance, architecture documents, established project conventions). Consult the user only when durable knowledge runs short; this consult is an entry-point action performed at shift start while the user is still present, before any unattended continuation. Apply the audience component-to-category judgment, the uplift-predicate judgments, and the derivation rule via `node internal/revise/rigor.js <audienceCategory> <firedUpliftCount>`; record every judgment boundary as a deviation entry with its basis.
