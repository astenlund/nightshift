...
Load `${CLAUDE_PLUGIN_ROOT}/skills/spec-agreement/spec-agreement.js`. When an active plan exists, also load `${CLAUDE_PLUGIN_ROOT}/internal/plan-binding.md` and `${CLAUDE_PLUGIN_ROOT}/internal/plan-binding.js`, call `establishPlanBinding` before the first plan read, and retain that full binding for the complete review lifecycle. Call `revalidatePlanBinding` before each authoritative plan read and before each reviewer or skeptic dispatch that receives plan-derived context. Consume only its captured plan bytes directly, including for `parsePlanContract`, acknowledgement seeds, overlap detection, and review payload content; never route plan authority through agreement CLI output or reopen the plan pathname. Obtain every governing seed by running `parsePlanContract` against those captured plan bytes and bind the agreement target to the selected stable whole-plan or section scope. Without an active plan, bind the target to the explicitly selected governing scope. Feed every explicit seed and parsed declaration through normal `resolveGoverningSet`; do not infer a governing artifact from recency or duplicate plan parsing, scope ordering, or candidate mapping in this profile.
...
- **Additional prompt rules**:
  ...
  - The holistic verifier for this profile holds itself to Dimension 4's Executed write paths rule: its clean LGTM names at least one live probe of every new write path, with the observed results, so the completion stamp never rests on read-only certification of code that mutates bytes.

- **Post-fix steps**: rebuild (build only, no tests) to confirm nothing is broken. If the explicit scope is `staged`, stage accepted fixes so the staged channel includes them in the next round. Do not stage otherwise. Commit each round's verified fixes at the round boundary when session and project conventions permit unprompted commits: use `git commit --fixup=<sha>` for a corrected commit range and a standalone commit otherwise. Where commits require explicit direction, leave fixes uncommitted and regenerate the cumulative patch against the working tree. Never commit while round agents are in flight. Where the project has no build step, the focused suites its instructions name for the changed files stand in for the rebuild. Judge that gate, build or suite, by the process exit status, never by a filtered summary: a chain that pipes the output through a line filter and continues on the filter's exit status commits a broken fix and launches the next round over it (observed: a filtered `# fail 5` line passed a commit gate). Redirect the output to a log and chain on the process's own exit.
...
7. **Executed write paths**: a new or changed path that writes, splices, or rewrites bytes (a file writer, a section replacer, a stamp appender) is certified only after a live probe that runs the real module against a temporary file or an in-memory buffer, covering one accepted write, an applied retry of the same write against the pre-write baseline, and a second write with a fresh baseline, and comparing the result against the code that will read it back (a parser, a checksum comparison, the reader of the written section). Reading the code is not the check (observed: eight cells certified a write path clean by reading it across four waves while a probing verifier found ten defects in it, each reproducible only by executing the module against crafted input).
