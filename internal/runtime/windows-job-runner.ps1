...
# Runner frames use exact, lexically ordered keys.
...
function Start-RunnerChild {
    ...
    if ($preAssignmentFailed) {
        ...
        if (-not ($terminated -and $signaled)) {
            return New-Object psobject -Property @{ Status = 'termination-unproven'; ProcessId = $child.ProcessId }
        }
        [void]$Interop.DeleteAttributeList($attributeList)
...
function Invoke-NightshiftJobRunner {
    try {
        ...
        if ($started.Status -eq 'termination-unproven') {
            Send-RunnerFrame ('{"detailCode":"termination","kind":"start-failed","pid":' + $started.ProcessId + '}')
            while ($true) {
        ...
        $stdinOpen = $true
        $closeInputRequested = $false
        $pendingWrites = New-Object 'System.Collections.Generic.Queue[object]'
        $pendingInputBytes = 0
        $nextInputOrdinal = 1
        ...
        while ($true) {
            ...
            while ($reader.Lines.Count -gt 0) {
                ...
                if (Test-RunnerExactKeys $frame @('dataBase64', 'kind', 'ordinal')) {
                    if ($frame.kind -cne 'host-input' -or -not $stdinOpen -or $closeInputRequested -or $frame.ordinal -ne $nextInputOrdinal) {
                        exit 2
                    }
                    $decoded = [Convert]::FromBase64String($frame.dataBase64)
                    $pendingInputBytes += $decoded.Length
                    if ($pendingInputBytes -gt $script:MaxPendingInputBytes) {
                        exit 2
                    }
                    $pendingWrites.Enqueue(@{ Task = $null; Bytes = $decoded; Flushing = $false; Ordinal = $frame.ordinal; Length = $decoded.Length })
                    $nextInputOrdinal = $nextInputOrdinal + 1
                } elseif (Test-RunnerExactKeys $frame @('kind')) {
                    if ($frame.kind -ceq 'close-input') {
                        if (-not $stdinOpen -or $closeInputRequested) {
                            exit 2
                        }
                        $closeInputRequested = $true
            ...
            # Writes must not block output pumping or a queued termination request.
            # Start one write at a time and acknowledge it after flush to retain order.
            while ($pendingWrites.Count -gt 0) {
                $pendingWrite = $pendingWrites.Peek()
                if ($null -eq $pendingWrite.Task) {
                    $pendingWrite.Task = $childStdin.WriteAsync($pendingWrite.Bytes, 0, $pendingWrite.Length)
                }
                if (-not $pendingWrite.Task.IsCompleted) {
                    break
                }
                [void]$pendingWrite.Task.GetAwaiter().GetResult()
                if (-not $pendingWrite.Flushing) {
                    $pendingWrite.Task = $childStdin.FlushAsync()
                    $pendingWrite.Flushing = $true
                    continue
                }
                Send-RunnerFrame ('{"kind":"input-accepted","ordinal":' + $pendingWrite.Ordinal + '}')
                $pendingInputBytes -= $pendingWrite.Length
                [void]$pendingWrites.Dequeue()
            }
            if ($closeInputRequested -and $stdinOpen -and $pendingWrites.Count -eq 0) {
                $stdinOpen = $false
                $childStdin.Dispose()
            }
