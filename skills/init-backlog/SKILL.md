...
4. **Apply.** Execute the approved actions. Never overwrite an existing top-level index file or an existing subdirectory's contents. For `unwrap` actions, capture `/nightshift:ready` output first, copy each file the check form lists into `.tmp/unwrap-backup/` (the only restore point when the backlog is git-ignored), run the repair form over `.claude/`, then capture the output again and compare, setting aside the hard-wrap notices, the `excerpt` fields, and the slice-name growth named under line discipline; any other difference means the unwrapper joined a line the parser reads as its own block, so restore that file from the backup and report it rather than keeping the rewrite.
