...
1. **Governing artifacts**: every canonical path as a direct link to the full artifact, in candidate order. Render each link as `[<linkText>](<linkTarget>)` taken verbatim from `locateSelection` (CLI operation `locate`, input `{projectRoot, path, selectorKind, selectors, sourceBytesHex}` where `path` must be project-relative in the same sense as a nominated governing path or the call fails as a `locate-input` structural error, result `{path, line, linkText, linkTarget}`) over the same stable baseline bytes as the candidate, never from a fresh search and never composed by hand. The controller owns the link rule: for an `index-entry` or `bullet-entry`, `linkText` is `<path>:<line>` (the one-based line where the selected entry starts) and `linkTarget` is the environment variable `NIGHTSHIFT_LINE_LINK_FORMAT` with `{path}` replaced by the absolute forward-slash path and `{line}` by the line (the ripgrep `--hyperlink-format` placeholders, so one editor-protocol format such as `subl://open?url=file:///{path}&line={line}` serves both tools); the CLI reads that variable itself and rejects a `linkFormat` field in its input, so the caller never decides whether it is set. When the variable is unset or empty, `linkTarget` is the bare absolute path: terminal link handlers open a file but ignore a `#L<line>` fragment or `:<line>` suffix, and a suffixed target silently stops opening at all. A `design-before-hardening` selection returns `line: null`, `linkText` equal to the path, and the bare absolute path as `linkTarget` regardless of the variable.
