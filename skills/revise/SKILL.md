...
The loop's own numbers are this retrospective's feedback signal: report the first-iteration confirmed-finding count per artifact type in the retrospective outcome, alongside the habits promoted or a stated null outcome ("no habit worth promoting"). The reported outcomes -- morning reports and session summaries -- are the durable cross-run trail; comparing against earlier runs' reported counts is how you judge whether promoted habits actually prevent findings.
