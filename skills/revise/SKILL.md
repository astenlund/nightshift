...
For a `needs-verification` cell, repair only findings still marked `needs-retry`. Validate each derived finding ID against its current owning cell and reviewer result order before selecting it. Resume the skeptic first when its session is valid; otherwise launch a fresh manual replacement. Merge its verdict without relaunching the reviewer or completed skeptics. Key reviewer repairs by dimension cell ID and skeptic repairs by the validated derived finding ID, never by optional session ID or display name.
