...
function skepticPrompt(d, f) {
  const finding = [
    ...
    ``,
    `## Verdict rules`,
    `CONFIRMED: the issue is real; cite the artifact evidence. REFUTED: the finding is wrong; cite the artifact evidence. JUDGMENT_CALL: not factually decidable (taste, balance, or priority), or decidable only by a live probe you cannot run. A finding asserting a runtime-owned literal or format (a UI string, a served payload shape, markup the host application renders) may NOT be REFUTED on repo or design-record evidence alone; the strongest repo-evidence verdict for it is JUDGMENT_CALL with a probe recommendation. When you run a live probe to verify or refute, replicate the execution scope and context of the code under review (module vs script scope, imported vs dot-sourced, the framework's real call path), not just the expression; a probe run in a different scope can prove behavior the real code path never exhibits. Check the artifact yourself; do not take the claimed evidence at face value.`,
  ]
  if (payloadFile) {
    return [
