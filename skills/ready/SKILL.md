...
Run the bundled ready.js with the absolute project root, or its .nightshift directory, as the argument. It returns ready, blocked, external and exploring work plus structural errors and notices. Report parser failures as failures, not an empty or clean backlog.

Present the ready set with concise context and dependencies. Keep blocked/external work distinguishable and Exploring drafts out of the ready set. Inspect linked source entries when recommending priorities, using the user goals and the invariant order of autonomy, quality and efficiency. Readiness does not authorize implementation: do not implement a recommendation on your own. Close the report by asking which items the user wants to take on, as a colleague's offer. Selecting items starts interactive investigation and readback, and the user decides separately when to hand over. This constraint is guidance for you and not for the user.
