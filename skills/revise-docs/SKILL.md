...
Preserve the four-index convention: shipped entries move to the corresponding history, dropping Requires/External; delivered slices are struck and the next slice gates advance; remove satisfied Requires references from other active entries. Retired proposals are recorded as retired, never shipped or fixed. Validate backlog changes with the real ready parser.

Report what changed and any remaining uncertainty. During a lifecycle, complete the session retrospective next, before follow-up triage. A standalone documentation request does not authorize unrelated implementation, publication or new instructions.
