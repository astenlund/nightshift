  ...
  test('the evaluation resolves the trusted git once with nonempty protected roots and threads it to the scenario runner wiring', async () => {
    ...
    try {
      const harness = createEvaluationHarness(acceptedScratch, {
        options: {
          commandFilesystem: fakeCommandFilesystem({ [outsideDirectory]: { kind: 'dir' }, [outsideGit]: { kind: 'file' } }),
          protectedRoots: [join(acceptedScratch, 'checkout')],
          runGitFactory: null,
        },
      })
      harness.options.ambientEnvironment.PATH = harness.options.ambientEnvironment.PATH + delimiter + outsideDirectory
      const evaluation = await hostBehavior.runEvaluation(harness.options)
