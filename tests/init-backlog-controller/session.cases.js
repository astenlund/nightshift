...
function runSessionCases(repositoryRoot) {
  ...
  test('transcript ordinals are contiguous, shared across kinds, and capacity-bounded before append', () => {
    ...
    const aggregate = transcript.toBuffer()
    const expectedAggregate = Buffer.concat(transcript.lines())
    assert.deepEqual(aggregate, expectedAggregate)
    aggregate.fill(0)
    assert.deepEqual(Buffer.concat(transcript.lines()), expectedAggregate, 'mutating the aggregate must not alter retained transcript lines')
  ...
  test('the proxy trace flushes each canonical line and enforces its capacity before append', () => {
    ...
    const aggregate = trace.toBuffer()
    assert.deepEqual(aggregate, flushed[0])
    aggregate.fill(0)
    assert.deepEqual(trace.lines()[0], flushed[0], 'mutating the aggregate must not alter retained proxy trace lines')
