...
function runRecoveryCases() {
  ...
  for (const prefix of [
    { name: 'backup stage creation', options: { onTransition: (() => { let count = 0; return (point) => { if (point === 'after-owner-stage-create' && ++count === 3) throw new Error('crash after backup stage creation') } })() } },
    { name: 'backup publication', options: { onPublished: (destination) => { if (destination.endsWith('.bak')) throw new Error('crash after backup publication') } } },
    { name: 'backup stage cleanup', options: { failAt: 'after-temporary-cleanup' } },
    { name: 'before target publication', options: { onPublished: (destination) => { if (destination.endsWith('.bak')) throw new Error('crash before target publication') } } },
  ]) {
    test(`unwrap response-loss resumes after ${prefix.name}`, () => {
      const fixture = unwrapFixture(1)
      try {
        assert.throws(() => publishApply(fixture.applyRequest, { currentInspection: fixture.carried, collectInspection: () => fixture.post, crash: true, ownerNonce: 'a'.repeat(32), ...prefix.options }), /crash|Injected publication failure/)
        const lock = JSON.parse(readFileSync(join(fixture.root, '.nightshift-init-backlog.lock'), 'utf8'))
        const backups = lock.temporaryPaths.filter((item) => item.endsWith('.bak'))
        assert.equal(backups.length, 1)
        const backup = join(fixture.root, ...backups[0].split('/'))
        if (prefix.name !== 'backup stage creation') assert.equal(existsSync(backup), true)
        const resumed = publishApply(fixture.applyRequest, { currentInspection: fixture.carried, collectInspection: () => fixture.post, resume: true, ownerNonce: 'a'.repeat(32) })
        assert.equal(resumed.operation, 'apply')
        const original = Buffer.from(fixture.carried.targets[0].contentBase64, 'base64')
        assert.deepEqual(readFileSync(join(fixture.root, fixture.actions[0].target)), Buffer.from(unwrapText(original.toString('utf8')), 'utf8'))
      } finally {
        rmSync(fixture.root, { force: true, recursive: true })
      }
    })
  ...
  test('unwrap response-loss rejects a successor that replaces the owned backup', () => {
    const fixture = unwrapFixture(1)
    try {
      assert.throws(() => publishApply(fixture.applyRequest, { currentInspection: fixture.carried, collectInspection: () => fixture.post, crash: true, ownerNonce: 'b'.repeat(32), onPublished: (destination) => { if (destination.endsWith('.bak')) throw new Error('crash after backup publication') } }), /crash/)
      const lock = JSON.parse(readFileSync(join(fixture.root, '.nightshift-init-backlog.lock'), 'utf8'))
      const backup = join(fixture.root, ...lock.temporaryPaths.find((item) => item.endsWith('.bak')).split('/'))
      writeFileSync(backup, Buffer.from('successor\n', 'utf8'))
      assert.throws(() => publishApply(fixture.applyRequest, { currentInspection: fixture.carried, collectInspection: () => fixture.post, resume: true, ownerNonce: 'b'.repeat(32) }), /temporary|changed|filesystem|publication/i)
  ...
  test('unwrap backup staging uses the normative snapshot-manifest-target name and stale-owner inspection recognizes the crash state', () => {
    const fixture = unwrapFixture(1)
    try {
      const manifestId = admitApplyManifest(fixture.applyRequest).manifestId
      assert.throws(() => publishApply(fixture.applyRequest, { currentInspection: fixture.carried, collectInspection: () => fixture.post, crash: true, ownerNonce: 'a'.repeat(32), onTransition: (() => { let count = 0; return (point) => { if (point === 'after-owner-stage-create' && ++count === 3) throw new Error('crash at backup stage') } })() }), /crash/)
      const lock = JSON.parse(readFileSync(join(fixture.root, '.nightshift-init-backlog.lock'), 'utf8'))
      const targetHash = sha256(Buffer.from(fixture.actions[0].target, 'utf8'))
      const expected = `.tmp/.nightshift-init-backlog-unwrap-${fixture.carried.snapshotId}-${manifestId}-${targetHash}.tmp`
      assert.ok(lock.temporaryPaths.includes(expected))
      writeCanonical(join(fixture.root, '.nightshift-init-backlog.lock'), { ...lock, temporaryPaths: [expected] })
      const inspected = inspectRecovery(request(fixture.root, 'stale-owner', '.nightshift-init-backlog.lock'), { killProcess: absentPid() })
      assert.equal(inspected.evidence.owner.temporaryStates.find((item) => item.target === expected).present, true)
    } finally {
      rmSync(fixture.root, { force: true, recursive: true })
    }
  })
