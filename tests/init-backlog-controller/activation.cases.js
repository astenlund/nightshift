...
function runActivationCases(repositoryRoot) {
  ...
  test('root-guidance digest refresh changes exactly the guidance.section manifest field', () => {
    ...
    const restoredManifest = manifestBytes.toString('utf8').split(entry.logicalSha256).join(PRE_ACTIVATION_GUIDANCE_DIGEST)
    assert.equal(sha256(Buffer.from(restoredManifest, 'utf8')), PRE_ACTIVATION_MANIFEST_SHA256, 'every manifest byte other than the guidance.section digest must remain unchanged')
    assert.throws(
