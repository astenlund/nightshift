...
function runActivationCases(repositoryRoot) {
  ...
  test('retired tracked-plan phrases are absent and shared plan policy is pinned', () => {
    ...
    assert.equal(countExact(liveBinding, 'For a Git root, verify that the plan\'s actual repository-relative path is ignored and untracked.'), 1, 'the shared owner must verify the selected repository plan path')
